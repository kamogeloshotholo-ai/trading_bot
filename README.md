...
- **Session-Aware Trading**: Adapts scanning frequency and symbol focus based on market sessions (Asian, London, New York, Weekend) for optimal performance during high/low activity periods.
- **Discord Alerts**: Sends real-time notifications to Discord when sweeps are detected or trades are executed.
...
Set the following environment variables for security and configuration:
...
- `DISCORD_WEBHOOK_URL`: URL for Discord webhook to send alerts (optional)
